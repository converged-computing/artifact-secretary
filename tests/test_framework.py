...
from secretary import Target, Inspector, ProfileTask, run_task, ToolSpec
...
class FakeSession:
    ...
    def __enter__(self): return Inspector(Target(self._root))
    def __exit__(self, *a): return False


class FakeRunner:
    def __init__(self, manifest): self.manifest = manifest
    async def converse(self, task): return self.manifest
    async def run_agent(self, system_prompt, user_prompt, tools, confirm_fn):
        byname = {t.name: t for t in tools}
        info = (await byname["inspect_elf"].handler({"path": "/lmp"}))["content"][0]["text"]
        import json as _j
        needed = _j.loads(info)["needed"]
        await byname["record_artifact"].handler(
            {"application": "lammps", "binary": "/lmp", "arch": "amd64", "needed": needed,
...
def test_profile_flow_direct_manifest():
    ...
    manifest = {"catalog": ["myreg/lammps:latest"], "goal": "profile", "keep_images": False}
    ...
    assert e.reproduce.pulled_by_us and e.artifacts and e.artifacts[0].application == "lammps"
    # round-trips through JSON
    import json
    reloaded = json.loads(lk.to_json())
    assert reloaded["version"].startswith("artifact-lookup/")
    print("OK profile flow -> lookup keyed by digest, artifact recorded, JSON round-trips")


def test_confirm_gate_on_action_tool():
    calls = []
    async def do(args): calls.append(args); return {"content": [{"type": "text", "text": "did it"}]}
    action = ToolSpec("submit", "submit something", {"x": int}, do, kind="action", confirm=True)
    ...
    assert "cancelled" in denied["content"][0]["text"] and not calls, "action ran despite denial!"
...
def test_setup_approval_can_abort():
    ...
    outcome = asyncio.run(run_task(task, FakeRunner({"catalog": []}),
                                   manifest={"catalog": []}, approve_fn=lambda m: False))
